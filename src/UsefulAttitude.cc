//////////////////////////////////////////////////////////////////////////////
/////  Port of pueo::UsefulAttitude to PUEO (now pueo::UsefulAttitude) 
/////                Cosmin Deaconu (cozzyd@kicp.uchicago.edu) 
/////
/////pueo::UsefulAttitude.cxx        ANITA event reading class                  /////
/////                                                                    /////
/////  Description:                                                      /////
/////    A simple class for utilising nav::Attitude objects                      /////
...
ClassImp(pueo::UsefulAttitude);

pueo::UsefulAttitude::UsefulAttitude()
  : nav::Attitude()
{
  //Default Constructor
  fIncludeGroupDelay=0;
  pitch = static_offsets::pitch;
  roll = static_offsets::roll;
  heading += static_offsets::heading;
  longitude = 0;
  latitude = 0;
  altitude = 0;
  fThetaWave=0;
...
pueo::UsefulAttitude::UsefulAttitude(const nav::Attitude *patPtr, bool staticPitchAndRoll)
  : nav::Attitude(*patPtr)
{
  fIncludeGroupDelay=0;
  ...
  {
    pitch = pueo::static_offsets::pitch;
    roll = pueo::static_offsets::roll;
    heading += pueo::static_offsets::heading;
  }
...
void pueo::UsefulAttitude::updateCartesianBalloonInfo(){
  ...
  if(longitude!=fBalloonLonCache ||
    ...
    double tmpx[3]; 
    pueo::GeomTool::Instance().getCartesianCoords(latitude,
						  longitude,
						  altitude,
						  tmpx);
    fBalloonPos.SetXYZ(tmpx[0],tmpx[1],tmpx[2]);
...
int pueo::UsefulAttitude::getSourceLonAndLatAltZero(Double_t phiWave, Double_t thetaWave, Double_t &sourceLon, Double_t &sourceLat) {
...
void pueo::UsefulAttitude::accountForPitchAndRollInPhiWaveThetaWave(Double_t& phiWave, Double_t& thetaWave) const {


  const pueo::GeomTool * geom = &pueo::GeomTool::Instance();
...
int pueo::UsefulAttitude::getSourceLonAndLatAtDesiredAlt(Double_t phiWave, Double_t thetaWave, Double_t &sourceLon, Double_t &sourceLat, Double_t desiredAlt = 0.0)
{
...
int pueo::UsefulAttitude::getSourceLonAndLatAtAlt(Double_t phiWave, Double_t thetaWave, Double_t &sourceLon, Double_t &sourceLat,Double_t &sourceAltitude) {
...
int pueo::UsefulAttitude::getSourceLonAndLatAtAlt2(Double_t phiWave, Double_t thetaWave, Double_t &sourceLon, Double_t &sourceLat,Double_t &sourceAltitude, Int_t maxLoopIterations, TVector3* sourcePosOptional) const
{
  // TGraph* grTest = NULL;
  // if(fDebug){
  ...
  const pueo::GeomTool* geom = &GeomTool::Instance();
...
int pueo::UsefulAttitude::traceBackToContinent3(Double_t phiWave, Double_t thetaWave,
...
int pueo::UsefulAttitude::getSourceLonAndLatAtAlt3(Double_t phiWave, Double_t thetaWave,
					    ...
					    double* deltaAltIfNoIntersectection, bool returnBestPositionIfNoIntersection) const 
{
  ...
  const pueo::GeomTool* geom = &pueo::GeomTool::Instance();
  Double_t balloonPosition[3];
  geom->getCartesianCoords(latitude, longitude, altitude, balloonPosition);
  const TVector3 balloonVector(balloonPosition);
  ...
  const TVector3 inwardNorm = balloonVector.Unit();
  ...
  for(int precise=0; precise <= 1; precise++){
    ...
    while(deltaAlt <= firstDeltaAlt){
      ...
      const TVector3 rayVector = i*deltaVector + balloonPosition; // ray position
 ...
 * @param thetaWave is the elevation angle (radians) theta=0 lies along the horizonal with -ve theta being up (the pueo::UsefulAttitude convention)
 * 
 * @return TVector3 containing a unit vector pointing to thetaWave/phiWave away from ANITA
 */

TVector3 pueo::UsefulAttitude::getUnitVectorAlongThetaWavePhiWave(double thetaWave, double phiWave) const{
  ...
  const TVector3 unitBalloon = TVector3(balloonPosition).Unit();
  testVector.Rotate(-testPhiWave, unitBalloon); // if we were to recalculate the phiWave expected, it would now point to 0
  ...
  testVector.Rotate(phiWave, unitBalloon); // if we were to recalculate the phiWave expected, it would now point to phiWave
  ...
  // a / sin(A) = t / sin(T)
  //
  // t = balloonPosition.Mag();
  // A = (pi/2 - thetaWave);
  // O = angle between ANITA and the test vector
  // T = pi - A - O
  // so...
  // a = sin(A) * t / (pi - A - O)
  TVector3 balloonVector(balloonPosition);
  
  double A = TMath::PiOver2() - thetaWave;
  double O = testVector.Angle(balloonPosition); //angle between the vectors
  double T = TMath::Pi() - A - O;
  double t = balloonVector.Mag();
  ...
  TVector3 deltaVec = testVector - balloonPosition;
...
void pueo::UsefulAttitude::getThetaAndPhiWave(Double_t sourceLon, Double_t sourceLat, Double_t sourceAlt, Double_t &thetaWave, Double_t &phiWave) {
  getThetaAndPhiWave2(sourceLon, sourceLat, sourceAlt, thetaWave, phiWave, &fSourcePos);
}



void pueo::UsefulAttitude::getThetaAndPhiWave2(Double_t sourceLon, Double_t sourceLat, Double_t sourceAlt, Double_t &thetaWave, Double_t &phiWave, TVector3* sourcePos) const {
...
void pueo::UsefulAttitude::getThetaAndPhiWaveCart(const TVector3 * sourcePosIn, Double_t & thetaWave, Double_t& phiWave) const 
{
  const GeomTool * geom = &GeomTool::Instance();
  TVector3 sourcePosCopy(*sourcePosIn); 
  TVector3* sourcePos = &sourcePosCopy; // kludge to avoid changing anything below
...
Double_t pueo::UsefulAttitude::getGroupDelay(Double_t phiToAntBoresight)
{
...
void pueo::UsefulAttitude::getThetaWaveAtBase(Double_t baseLon, Double_t baseLat, Double_t baseAlt, Double_t &thetaWave) {
  
  //gets the theta from a base to the balloon - can check if we are beyond horizon

  Double_t pSource[3]={0};
  GeomTool::Instance().getCartesianCoords(TMath::Abs(baseLat),baseLon,baseAlt,pSource);
...
Double_t pueo::UsefulAttitude::getDeltaTExpected(Int_t ant1, Int_t ant2,Double_t sourceLon, Double_t sourceLat, Double_t sourceAlt)
{
...
Double_t pueo::UsefulAttitude::getDeltaTExpected(Int_t ant1, Int_t ant2,Double_t phiWave, Double_t thetaWave)
{
...
Double_t pueo::UsefulAttitude::getDeltaTExpected(Int_t ant1,Int_t ant2,Double_t cosPhi,Double_t sinPhi,Double_t cosTheta,Double_t sinTheta)
{  
  Double_t x1,y1,z1;
  const GeomTool *geom = &GeomTool::Instance();
...
Double_t pueo::UsefulAttitude::getDeltaTExpectedOpt(Int_t ant1, Int_t ant2,Double_t sourceLon, Double_t sourceLat, Double_t sourceAlt, Double_t *deltaR, Double_t *deltaZ, Double_t *deltaPhi)
{
  Double_t tempTheta,tempPhi;
  if(fSourceAltitude!=sourceAlt || fSourceLongitude!=sourceLon || fSourceLatitude!=sourceLat)
    getThetaAndPhiWave(sourceLon,sourceLat,sourceAlt,tempTheta,tempPhi);
...
Double_t pueo::UsefulAttitude::getDeltaTExpectedSeaveyOpt(Int_t ant1, Int_t ant2,Double_t sourceLon, Double_t sourceLat, Double_t sourceAlt, Double_t *deltaR, Double_t *deltaZ, Double_t *deltaPhi)
{
  Double_t tempTheta,tempPhi;
  if(fSourceAltitude!=sourceAlt || fSourceLongitude!=sourceLon || fSourceLatitude!=sourceLat){
    getThetaAndPhiWave(sourceLon,sourceLat,sourceAlt,tempTheta,tempPhi);
...
Double_t pueo::UsefulAttitude::getDistanceFromSource(Double_t sourceLat, Double_t sourceLong, Double_t sourceAlt) const 
{
  // static Double_t pTaylor[3]={0};
  Double_t pTaylor[3]={0};  
  GeomTool::Instance().getCartesianCoords(sourceLat,
						sourceLong,
						sourceAlt,
						pTaylor);
  Double_t s2 = ((pTaylor[0]-fBalloonPos.x())*(pTaylor[0]-fBalloonPos.x()) +
		 (pTaylor[1]-fBalloonPos.y())*(pTaylor[1]-fBalloonPos.y()) +
		 (pTaylor[2]-fBalloonPos.z())*(pTaylor[2]-fBalloonPos.z()));
...
UInt_t pueo::UsefulAttitude::getTriggerTimeNsFromSource(Double_t sourceLat, Double_t sourceLong, Double_t sourceAlt) const
{
...
/*
Double_t pueo::UsefulAttitude::getAngleBetweenPayloadAndSource(Double_t sourceLon, Double_t sourceLat, Double_t sourceAlt) { //ACG additional function
  const GeomTool * geom = &GeomTool::Instance();
 ...
 *
 * Uses the timeOfDay variable from inside the nav::Attitude to get the sun's azimuth position relative to north.
 * i.e. returns -1*heading of the sun, as payload phi increases anti-clockwise and heading increases clockwise.
 * The more useful quantity is probably returned by pueo::UsefulAttitude::getAzimuthOfSun().
 */
Double_t pueo::UsefulAttitude::getAzimuthOfSunRelativeToNorth() const{
 ...
 */
Double_t pueo::UsefulAttitude::getAzimuthOfSun() const {
  Double_t sunAngle = pueo::UsefulAttitude::getAzimuthOfSunRelativeToNorth();
 ...
 */
Double_t pueo::UsefulAttitude::getDifferencePointingToSun(Double_t phiAngle, Bool_t inputInDegrees) const {

  if(phiAngle>9999 || phiAngle<-9999){
    std::cerr << "pueo::UsefulAttitude::getDifferencePointingToSun() " << phiAngle << " ignore\n";
    return -9999.;
  }

  Double_t sunAngle = pueo::UsefulAttitude::getAzimuthOfSun();
 ...
 */
void pueo::UsefulAttitude::getSunPosition(Double_t& phiDeg, Double_t& thetaDeg) const{
...
int pueo::UsefulAttitude::traceBackToContinent(Double_t phiWave, Double_t thetaWave,
...
Double_t pueo::UsefulAttitude::getReflectionAngle(Double_t plAlt, Double_t el, Double_t imAlt){
...
Double_t pueo::UsefulAttitude::getReflectionAngle(Double_t el, Double_t imAlt) const {
  return getReflectionAngle(this->altitude, el, imAlt);
}


int pueo::UsefulAttitude::astronomicalCoordinates(Double_t phiWave, Double_t thetaWave, Double_t * RA_ptr, Double_t * dec_ptr, Double_t * l_ptr, Double_t * b_ptr) const
{
...
int pueo::UsefulAttitude::fromRADec(int N, const Double_t * RA, const Double_t * dec, Double_t * phi, Double_t *theta) const
{
#if ROOT_VERSION_CODE < ROOT_VERSION(6,0,0)
  std::cerr << "Your  version of ROOT does not support all the features of eventReaderRoot\n";
  return 0;
...
int pueo::UsefulAttitude::fromRADec(Double_t RA, Double_t dec, Double_t * phi, Double_t *theta) const
{
#if ROOT_VERSION_CODE < ROOT_VERSION(6,0,0)
  std::cerr << "Your  version of ROOT does not support all the features of eventReaderRoot\n";
  return 0;
...
void pueo::UsefulAttitude::getThetaAndPhiWaveOfRayAtInfinity(const TVector3 & p0, const TVector3 & v0, Double_t & theta, Double_t & phi, Bool_t plus, Double_t eps, Double_t tstep, TVector3 * testp) const 
{
  tstep = fabs(tstep); 
  if (!plus) tstep *= -1; 
