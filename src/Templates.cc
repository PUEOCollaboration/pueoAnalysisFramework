...
//---------------------------------------------------------------------------------------------------------
/** pueo::TemplateSummary
 * @brief Default Constructor
 *
 * Default constructor for ROOT
 */
pueo::TemplateSummary::TemplateSummary() {
  zeroInternals();
}
...
void pueo::TemplateSummary::zeroInternals(){

  for (Int_t poli=0; poli<k::NUM_POLS; poli++) {
...
//---------------------------------------------------------------------------------------------------------
/** pueo::TemplateMachine
 * @brief Default Constructor
 *
 * Default constructor for ROOT
 */
pueo::TemplateMachine::TemplateMachine(int inLength)
    : length(inLength), dT(0.1)
{
  ...
  if(pueo::version::get() == 4) fillNotchConfigs();

  zeroInternals();
}

pueo::TemplateMachine::~TemplateMachine() {
  zeroInternals();
}


void pueo::TemplateMachine::zeroInternals() {
...
void pueo::TemplateMachine::fillNotchConfigs()
{
  int tempTime;
  std::string tempStr;
  char* installDir = getenv("PUEO_UTIL_INSTALL_DIR");
  std::ifstream inf(Form("%s/share/pueo::AnalysisFramework/responses/TUFFs/index.txt", installDir));
...
void pueo::TemplateMachine::getImpulseResponseTemplate(int version) {
  ...
  if(version == 4)
  {
    name = Form("%s/share/pueoAnalysisFramework/responses/TUFFs/averages/%s.imp", installDir, fNotchStr.c_str());
  }
  else name = Form("%s/share/pueoAnalysisFramework/responses/SingleBRotter/all.imp", installDir);
  ...
  TGraph *grTemplate = FFTtoolsAnnex::normalizeWaveform(grTemplatePadded);
  delete grTemplatePadded;
...
void pueo::TemplateMachine::getCRTemplates(int version) {

  char* installDir = getenv("PUEO_UTIL_INSTALL_DIR");
  TString fname;
  if(version == 4 && !fUseAverageCRTemplate)
  {
    fname = Form("%s/share/pueoAnalysisFramework/templates/crTmpltsA4_%s.root", installDir, fNotchStr.c_str());
  }
  else if(version == 4 && fUseAverageCRTemplate)
  {
    fname = Form("%s/share/pueoAnalysisFramework/templates/averageA4CR_%s.txt", installDir, fNotchStr.c_str());
  }
  else if(version != 4 && fUseAverageCRTemplate)
  {
    fname = Form("%s/share/pueoAnalysisFramework/templates/averageA3CR.txt", installDir);
  }
  else fname = Form("%s/share/pueoAnalysisFramework/templates/crTmpltsA3.root", installDir);
  ...
  if(fUseAverageCRTemplate)
  {
    ...
    //this second one is for polarity tests 
    fname = Form("%s/share/pueo::AnalysisFramework/templates/crTmpltsA4_%s.root", installDir, fNotchStr.c_str());
    TFile *inFile = TFile::Open(fname.Data());
    std::stringstream name;

    //want to get graphs 13 through 24 (like in makeTemplate.C)
    int wave = 13; //peak seems to be at around the 13th one, then by 23 it is basically zero
    name.str("");
    name << "disp" << wave;
    TGraph *grTemplateRaw = (TGraph*)inFile->Get(name.str().c_str());
    ...
    //waveforms are super long so we can just cut it to the window dimentions
    int peakHilb = -1;
    grTemplateCut = WindowingTools::windowCut(grTemplateRaw,length);
    delete grTemplateRaw;
    ...
    //and finally normalize it (last step!)
    grTemplate = FFTtoolsAnnex::normalizeWaveform(grTemplateCut);
    delete grTemplateCut;
    ...
    theCRTemplates[1] = grTemplate;
    theCRTemplateFFTs[1] = theTemplateFFT;
  }
    return;
}
...
void pueo::TemplateMachine::getWaisTemplate(int version) {
 
  char* installDir = getenv("PUEO_UTIL_INSTALL_DIR");
  std::stringstream name;
  name.str("");
  if(version::get() == 4) name << installDir << "/share/pueoAnalysisFramework/templates/waisTemplateA4.root";
  else name << installDir << "/share/pueoAnalysisFramework/templates/waisTemplateA3.root";
  TFile *inFile = TFile::Open(name.str().c_str());
  TGraph *grTemplateRaw = (version::get() == 4) ? (TGraph*)inFile->Get("wais") : (TGraph*)inFile->Get("wais01TH");
  ...
  int peakHilb = -1;
  TGraph *grTemplateCut = (version::get() == 4) ? new TGraph(grTemplateRaw->GetN(), grTemplateRaw->GetX(), grTemplateRaw->GetY()) : WindowingTools::windowCut(grTemplateRaw,length);
  //TGraph *grTemplateCut = WindowingTools::windowCut(grTemplateRaw,length);
  delete grTemplateRaw;
...
void pueo::TemplateMachine::loadTemplates(unsigned int evTime, int version) {
...
void pueo::TemplateMachine::doTemplateAnalysis(const AnalysisWaveform *waveform, int poli, int dir, pueo::TemplateSummary *templateSummary, bool do_impulse, bool do_wais, bool do_cr) {
  /* copied out of templateSearch.cc */


  if (!isTmpltsLoaded()) {
    std::cout << "Error in pueo::TemplateAnalyzer::doTemplateAnalysis(): pueo::TemplateMachine says it has no templates!" << std::endl;
    std::cout << "      Try doing pueo::TemplateMachine::loadTemplates() at some point maybe? " << std::endl;
    return;
  }
  ...
  TGraph *normCoherent = FFTtoolsAnnex::normalizeWaveform(coherentPad);
  delete coherentPad;
  ...
  if (do_impulse) 
  {
    dCorr = FFTtoolsAnnex::getCorrelationFromFFT(length,theImpTemplateFFT,coherentFFT);
  ...
  if (do_wais)
  {
    dCorr = FFTtoolsAnnex::getCorrelationFromFFT(length,theWaisTemplateFFT,coherentFFT);
    maxValue = TMath::MaxElement(length,dCorr);
    minValue = TMath::Abs(TMath::MinElement(length,dCorr));
    if (TMath::Max(maxValue,minValue) == maxValue) {
      value = maxValue;
  ...
  if(!fUseAverageCRTemplate)
  {
    for (int i=0; i<numCRTemplates; i++) {
      if (!do_cr) break; 
      dCorr = FFTtoolsAnnex::getCorrelationFromFFT(length,theCRTemplateFFTs[i],coherentFFT);
      maxValue = TMath::MaxElement(length,dCorr);
      minValue = TMath::Abs(TMath::MinElement(length,dCorr));
      if (TMath::Max(maxValue,minValue) == maxValue) {
        value = maxValue;
  ...
  {
    dCorr = FFTtoolsAnnex::getCorrelationFromFFT(length,theCRTemplateFFTs[0],coherentFFT);
    maxValue = TMath::MaxElement(length,dCorr);
    minValue = TMath::Abs(TMath::MinElement(length,dCorr));
    if (TMath::Max(maxValue,minValue) == maxValue) {
      value = maxValue;
...
void pueo::TemplateMachine::doDeconvolvedTemplateAnalysis(const AnalysisWaveform *waveform, 
							 const pueo::DeconvolutionMethod *deconv,
							 int poli, int dir, pueo::TemplateSummary *templateSummary,
  ...
  TGraph *normDeconvolved = FFTtoolsAnnex::normalizeWaveform(deconvolvedPad);
  ...
  if (do_impulse)
  {
    dCorr = FFTtoolsAnnex::getCorrelationFromFFT(length,theImpTemplateFFT_deconv,deconvolvedFFT);
    maxValue = TMath::MaxElement(length,dCorr);
    minValue = TMath::Abs(TMath::MinElement(length,dCorr));
    if (TMath::Max(maxValue,minValue) == maxValue) {
      value = maxValue;
  ...
  if (do_wais) 
  {
    dCorr = FFTtoolsAnnex::getCorrelationFromFFT(length,theWaisTemplateFFT_deconv,deconvolvedFFT);
    maxValue = TMath::MaxElement(length,dCorr);
    minValue = TMath::Abs(TMath::MinElement(length,dCorr));
    if (TMath::Max(maxValue,minValue) == maxValue) {
      value = maxValue;
  ...
  if(!fUseAverageCRTemplate)
  {
    for (int i=0; i<numCRTemplates; i++) {
      if (!do_cr) break; 
      dCorr = FFTtoolsAnnex::getCorrelationFromFFT(length,theCRTemplateFFTs_deconv[i],deconvolvedFFT);
      maxValue = TMath::MaxElement(length,dCorr);
      minValue = TMath::Abs(TMath::MinElement(length,dCorr));
      if (TMath::Max(maxValue,minValue) == maxValue) {
        value = maxValue;
  ...
  if(fUseAverageCRTemplate && do_cr)
  {
    if(fDoWindow) {
      ...
      dCorr = FFTtoolsAnnex::getCorrelationFromFFT(length,theCRTemplateFFTs_deconv[0],deconvolvedFFT);
      maxValue = TMath::MaxElement(length,dCorr);
      minValue = TMath::Abs(TMath::MinElement(length,dCorr));
      if (TMath::Max(maxValue,minValue) == maxValue) {
        value = maxValue;
...
void pueo::TemplateMachine::writeTemplatesToFile(TFile *outFile) {

  if (!isTmpltsLoaded() || !isTmpltsDeconv()) {
    std::cout << "Error in pueo::TemplateAnalyzer::writeTemplatesToFile(): pueo::TemplateMachine says it is missing templates!" << std::endl;
    std::cout << "      Try doing pueo::TemplateMachine::loadTemplates() and deconvolveTempaltes() at some point maybe? " << std::endl;
    return;
  }
...
*/
TGraph *pueo::FFTtoolsAnnex::normalizeWaveform(TGraph *inGraph) {
...
double *pueo::FFTtoolsAnnex::getCorrelationFromFFT(int length,const FFTWComplex *theFFT1, const FFTWComplex *theFFT2) 
{
...
TGraph *pueo::WindowingTools::windowCut(TGraph *inGraph,int length) {
...
TGraph *pueo::WindowingTools::windowDispersed(TGraph *inGraph, int &peakHilbertLoc) {  
    return windowWave(inGraph,peakHilbertLoc,
		      500,750,20,20);
  }
  
  
TGraph * pueo::WindowingTools::windowDispersed(TGraph *inGraph) {
    //overload!
    int tempPeak = -1;
...
TGraph * pueo::WindowingTools::windowEField(TGraph *inGraph, int &peakHilbertLoc) {
  return windowWave(inGraph,peakHilbertLoc,
		    30,30,2,2);
}


TGraph * pueo::WindowingTools::windowEField(TGraph *inGraph) {
  //overload!
  int tempPeak = -1;
...
TGraph * pueo::WindowingTools::windowWave(TGraph *inGraph, int &peakHilbertLoc,
